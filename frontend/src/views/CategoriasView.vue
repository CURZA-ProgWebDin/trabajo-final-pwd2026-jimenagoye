  ...
  <div class="crud-container">
    <div class="crud-card">
      <div class="crud-header">
        <h1><span class="icon">&#9670;</span> Gesti&oacute;n de Categor&iacute;as</h1>
        <button class="btn-add" @click="abrirModal()">+ Nueva Categor&iacute;a</button>
      </div>
      ...
      <table v-else class="crud-table">
        ...
        <tbody>
          <tr v-for="cat in store.lista" :key="cat.id">
            <td class="td-id">{{ cat.id }}</td>
            <td>{{ cat.nombre }}</td>
            <td>{{ cat.descripcion || '—' }}</td>
            <td class="td-actions">
              <button class="btn-icon edit" title="Editar" @click="abrirModal(cat)">&hellip;</button>
              <button class="btn-icon delete" title="Eliminar" @click="confirmarEliminar(cat)">&times;</button>
            </td>
          </tr>
        </tbody>
      </table>
    ...
    <div v-if="modal" class="modal-overlay" @click.self="cerrarModal">
      <div class="modal-card">
        <div class="modal-header">
          <h2>{{ editando ? 'Editar Categor&iacute;a' : 'Nueva Categor&iacute;a' }}</h2>
          <button class="btn-close" @click="cerrarModal">&times;</button>
        </div>
        <form @submit.prevent="guardar">
          <div class="form-group">
            <label>Nombre</label>
            <input v-model="form.nombre" type="text" placeholder="Nombre de la categor&iacute;a" required />
          </div>
          <div class="form-group">
            <label>Descripci&oacute;n</label>
            <input v-model="form.descripcion" type="text" placeholder="Descripci&oacute;n opcional" />
          </div>
...
async function guardar() {
  ...
  try {
    if (editando.value) {
      await store.actualizarCategoria(editandoId.value, { nombre: form.nombre, descripcion: form.descripcion });
      successMsg.value = 'Categor&iacute;a actualizada correctamente.';
    } else {
      await store.crearCategoria({ nombre: form.nombre, descripcion: form.descripcion });
      successMsg.value = 'Categor&iacute;a creada correctamente.';
    }
    cerrarModal();
  } catch (e) {
...
async function eliminar(id) {
  try {
    ...
    successMsg.value = 'Categor&iacute;a eliminada correctamente.';
