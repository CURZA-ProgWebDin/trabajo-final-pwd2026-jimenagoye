  ...
  <div class="crud-container">
    <div class="crud-card">
      <div class="crud-header">
        <h1><span class="icon">&#9632;</span> Gesti&oacute;n de Proveedores</h1>
        <button class="btn-add" @click="abrirModal()">+ Nuevo Proveedor</button>
      </div>
      ...
      <div v-else-if="store.lista.length === 0" class="empty">
      ...
      <table v-else class="crud-table">
        ...
        <tbody>
          <tr v-for="prov in store.lista" :key="prov.id">
            <td class="td-id">{{ prov.id }}</td>
            <td>{{ prov.nombre }}</td>
            <td>{{ prov.contacto || '&mdash;' }}</td>
            <td>{{ prov.telefono || '&mdash;' }}</td>
            <td>{{ prov.email || '&mdash;' }}</td>
            <td class="td-actions">
              <button class="btn-icon edit" title="Editar" @click="abrirModal(prov)">&hellip;</button>
              <button class="btn-icon delete" title="Eliminar" @click="confirmarEliminar(prov)">&times;</button>
            </td>
          </tr>
        </tbody>
      </table>
      ...
      <div class="modal-card">
        ...
        <form @submit.prevent="guardar">
          <div class="form-group">
            <label>Nombre</label>
            <input v-model="form.nombre" type="text" placeholder="Nombre del proveedor" required />
          </div>
          <div class="form-group">
            <label>Contacto</label>
            <input v-model="form.contacto" type="text" placeholder="Persona de contacto" />
          </div>
          <div class="form-group">
            <label>Tel&eacute;fono</label>
            <input v-model="form.telefono" type="text" placeholder="Tel&eacute;fono" />
          </div>
          <div class="form-group">
            <label>Email</label>
...
const form = reactive({ nombre: '', contacto: '', telefono: '', email: '' });
...
async function guardar() {
  ...
  successMsg.value = '';
  try {
    const payload = { nombre: form.nombre, contacto: form.contacto, telefono: form.telefono, email: form.email };
    if (editando.value) {
      await store.actualizarProveedor(editandoId.value, payload);
      successMsg.value = 'Proveedor actualizado correctamente.';
    } else {
      await store.crearProveedor(payload);
      successMsg.value = 'Proveedor creado correctamente.';
    }
    cerrarModal();
  } catch (e) {
...
function confirmarEliminar(prov) {
  confirmando.value = prov;
  errorMsg.value = '';
  successMsg.value = '';
}
...
async function eliminar(id) {
  try {
    ...
    successMsg.value = 'Proveedor eliminado correctamente.';
...
.th-actions, .td-actions { text-align: right; width: 100px; }
