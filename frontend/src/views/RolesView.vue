  ...
  <div class="crud-container">
    <div class="crud-card">
      <div class="crud-header">
        <h1><span class="icon">&#9881;</span> Gesti&oacute;n de Roles</h1>
        <button class="btn-add" @click="abrirModal()">+ Nuevo Rol</button>
      </div>
      ...
      <div v-else-if="store.lista.length === 0" class="empty">No hay roles registrados.</div>
      ...
      <div v-else class="table-wrapper">
        <table class="crud-table">
          ...
          <tbody>
            <tr v-for="r in store.lista" :key="r.id">
              <td class="td-id">{{ r.id }}</td>
              <td><span class="badge" :class="r.nombre">{{ r.nombre }}</span></td>
              <td class="td-actions">
                <button class="btn-icon edit" title="Editar" @click="abrirModal(r)">&hellip;</button>
                <button class="btn-icon delete" title="Eliminar" @click="confirmarEliminar(r)">&times;</button>
              </td>
            </tr>
          </tbody>
        </table>
      ...
      <div class="modal-card">
        ...
        <form @submit.prevent="guardar">
          <div class="form-group">
            ...
            <input v-model="form.nombre" type="text" placeholder="admin / operador" required />
          </div>
    ...
    <div v-if="confirmando" class="modal-overlay" @click.self="confirmando = null">
      <div class="modal-card confirm-card">
        ...
        <p>&iquest;Est&aacute;s segura de eliminar el rol <strong>{{ confirmando.nombre }}</strong>?</p>
        <div class="form-actions">
...
async function guardar() {
  ...
  try {
    if (editando.value) {
      await store.updateRole(editandoId.value, { nombre: form.nombre });
      successMsg.value = 'Rol actualizado correctamente.';
    } else {
      await store.createRole({ nombre: form.nombre });
      successMsg.value = 'Rol creado correctamente.';
    }
    cerrarModal();
  } catch (e) {
    errorMsg.value = e.response?.data?.message || e.message || 'Error al guardar rol';
  }
}
...
async function eliminar(id) {
  try {
    ...
    successMsg.value = 'Rol eliminado correctamente.';
    confirmando.value = null;
  } catch (e) {
    errorMsg.value = e.response?.data?.message || e.message || 'Error al eliminar rol';
    confirmando.value = null;
  }
}
</script>
...
.loading, .empty { text-align: center; padding: 2rem; color: #94a3b8; font-size: 0.95rem; }
